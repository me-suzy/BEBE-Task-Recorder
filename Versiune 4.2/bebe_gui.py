...
class BebeGUI:
    ...
    def toggle_mini_mode(self):
        """Toggle Mini UI Mode - fereastră compactă cu butoane esențiale"""
        if hasattr(self, 'mini_window') and self.mini_window.winfo_exists():
            # Închide Mini Mode
            self.mini_window.destroy()
            self.root.deiconify()  # Arată fereastra principală
            return

        # Creează Mini Mode window
        self.mini_window = tk.Toplevel(self.root)
        self.mini_window.title("BEBE Mini Mode")
        self.mini_window.geometry("300x300")
        self.mini_window.resizable(False, False)

        # Pozitionare în colțul din dreapta jos
        screen_width = self.mini_window.winfo_screenwidth()
        screen_height = self.mini_window.winfo_screenheight()
        x = screen_width - 320
        y = screen_height - 340
        self.mini_window.geometry(f"300x300+{x}+{y}")

        # Always on top
        self.mini_window.attributes('-topmost', True)

        # Frame principal
        main_mini_frame = ttk.Frame(self.mini_window, padding="15")
        main_mini_frame.pack(fill=tk.BOTH, expand=True)

        # Titlu
        ttk.Label(main_mini_frame, text="🎬 Quick Controls",
                 font=("Arial", 11, "bold")).pack(pady=(0, 10))

        # Butoane esențiale cu font mare
        btn_style = {'width': 20}

        ttk.Button(main_mini_frame, text="▶️ Play (Space)",
                  command=self.play_task, **btn_style).pack(pady=5)

        ttk.Button(main_mini_frame, text="⏸️ Pause (F10)",
                  command=self.pause_playback, **btn_style).pack(pady=5)

        ttk.Button(main_mini_frame, text="⏹️ Stop (Esc/F9)",
                  command=self.stop_playback, **btn_style).pack(pady=5)

        ttk.Button(main_mini_frame, text="⏺️ Record (F1)",
                  command=self.record_task, **btn_style).pack(pady=5)

        ttk.Separator(main_mini_frame, orient='horizontal').pack(fill=tk.X, pady=10)

        ttk.Button(main_mini_frame, text="🔳 Full Mode",
                  command=self.toggle_mini_mode, **btn_style).pack(pady=5)
        ...
        # Minimizează fereastra principală
        self.root.withdraw()

        self.logger.info("Mini Mode activated")
